...
def _research_node(state: ResearchState, config: RunnableConfig):
...
def _summarize_node(state: ResearchState, config: RunnableConfig):
...
def build_research_graph() -> None:
    ...
    GraphRegistry.register("research_chain", graph.compile())
