...
def get_working_dir(runtime: ToolRuntime[Any, Any]) -> str:
    ...
    For sandbox backends the workspace ``locator`` is a sandbox ID and is not
    a usable filesystem path. Prefer ``metadata["root"]`` so the value reflects
    the actual cwd used by the backend (which may sit inside a cloned
    repository).
    """
